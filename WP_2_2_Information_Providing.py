...
def destination_chosen(id):
    ...
    """
    carts, carts_total, carts_num = [],0,0
    ...
    return render_template("specificDestination.html", destination=destination_object, attractions=attractions,
                           accommodations=accommodations,carts = carts,carts_total=carts_total,carts_num = len(carts))
...
def detail_page_destination(id):
    ...
    return render_template("detail_destination.html", destination=destination_object,carts = carts,carts_total=carts_total,carts_num = len(carts))
...
def detail_page_attraction(id):
    ...
    if(id != "None"):
        ...
        score = attract_object.score
        tags = get_tags(attract_object.tags)
        attract_object.view = attract_object.id + 1
        db.session.add(attract_object)
        db.session.commit()

        return render_template("detail_attraction.html", attraction=attract_object, reviews=reviews,carts = carts,carts_total=carts_total,carts_num = len(carts),score = score,tags = tags)
...
def detail_page_accommodation(id):
    ...
    if( id != 'None'):
        ...
        score = accommodation_object.score
        tags = get_tags(destination.tags)
        accommodation_object.view = accommodation_object.view + 1
        db.session.add(accommodation_object)
        db.session.commit()

        return render_template("detail_accommodation.html", accommodation=accommodation_object, reviews=reviews,
                               destination=destination,carts = carts,carts_total=carts_total,carts_num = len(carts),score = score,tags = tags)
...
@wp_2.route("/search/", methods=['GET', 'POST'])
def search_page_full():
    carts, carts_total, carts_num = [], 0, 0
    if current_user.is_authenticated:
        carts,carts_total,carts_num=get_cart()
    ...
    return render_template("search_page.html", d_list=d_list, attract_list=attract_list, accommo_list=accommo_list,
                           current_keyword="",carts = carts,carts_total=carts_total,carts_num = len(carts))
...
def search_page(key_word):
    ...
    if current_user.is_authenticated:
        carts,carts_total,carts_num=get_cart()
    if (key_word == None or key_word == "" or key_word == "None"):
        d_list = db.session.query(Destination).all()
        attract_list = db.session.query(Attraction).all()
        accommo_list = db.session.query(Accommodation).all()
    else:
    ...
    return render_template("search_page.html", d_list=d_list, attract_list=attract_list, accommo_list=accommo_list,
                           current_keyword=key_word,carts = carts,carts_total=carts_total,carts_num = len(carts))
...
def search_for_everything(search_key_word):
    ...
    attraction_list_by_country = db.session.query(Attraction).join(Destination).filter(
        Destination.country.like(search_key_word)).all()
    ...
    accommodation_list_by_location = db.session.query(Accommodation).filter(
        Accommodation.location.like(search_key_word)).all()
    accommodation_list_by_info = db.session.query(Accommodation).filter(Accommodation.info.like(search_key_word)).all()
    accommodation_list_by_country = db.session.query(Accommodation).join(Destination).filter(
        Destination.country.like(search_key_word)).all()
...
def get_tags(tag_ids):
    res_list = []
    ids_list = tag_ids.split(",")
    for id in ids_list:
        if(id == "1"):
            res_list.append("Natural scenery")
        elif (id == "2"):
            res_list.append("Historical and cultural")
        elif (id == "3"):
            res_list.append("Theme park")
        elif (id == "4"):
            res_list.append("Culinary experience")
        elif (id == "5"):
            res_list.append("Luxury")
        elif (id == "6"):
            res_list.append("Landmarks and monuments")
        elif (id == "7"):
            res_list.append("Nighttime sightseeing")
        elif (id == "8"):
            res_list.append("Sports and leisure")
        elif (id == "9"):
            res_list.append("Adventure and exploration")
